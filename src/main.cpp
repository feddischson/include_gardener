...
Solver::Ptr init_options(int argc, char* argv[], Options* opts) {
   ...
   po::store(po::command_line_parser(argc, argv)
                 .options(desc)
                 .allow_unregistered()  // Don't catch unknown options until we
                                        // add language-dependent options
                 .run(),
             vm);
   po::notify(vm);
   ...
   if (nullptr == solver) {
      cerr << "Error: Unsupported language \"" << opts->language << "\""
           << "\n";
      exit(-1);
   }

   // Add the vector options
   desc.add_options()("process-path,P",
